...
Route::get('welcome', [PlaylistController::class, "save_playlist"])->name("playlist.save_playlist");
...
Route::get('/playlist/details/{name}', function (Request $request, $name) {
    ...
    $playlist["songs"][0] = $request->session()->pull('key', 'default');

    foreach($playlist["songs"] as $song_id){
        $song= Song::where('song_id', '=', $song_id)->get();
        $playlist["songs"][$song_id] = $song;
    }
    //return $playlist[0]["songs"];
    //$play_songs =$playlist[0]["songs"][$song_id];
